(program
 ...
 (define-label
  $lambda49
  (lambda ($env rv25)
    (set-then! g$sum rv25 (app* (env-ref $env_t43 $env k24) (void)))))
 (define-label
  $lambda50
  (lambda ($env k24)
    ((cps +)
     g$sum
     g$sum
     (make-closure (lambda-label $lambda49) (make-env $env_t43 (k24 k24))))))
 (define-label
  $lambda51
  (lambda ($env continue k23)
    (set-then!
     ...
     (app* (make-closure (lambda-label $lambda50) (make-env $env_t39)) k23))))
 (define-label
  $lambda52
  (lambda ($env i14 k22)
    (app*
 ...
 (define-label
  ...
  (lambda ($env break k14)
    (app*
     ...
     k14)))
 (define-label $lambda54 (lambda ($env rv26) ((cps py-print) g$sum $halt)))
 (define-label
  $lambda38
  (lambda ($env x k) (app* (env-ref $env_t38 $env cc) x)))
 (define-label
