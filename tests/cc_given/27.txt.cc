               ...
               (make-closure
                (lambda ($env $seq14 $loop15 k15)
                  (app*
                   ...
                   (make-closure
                    (lambda ($env rv17)
                      (app* (env-ref $env_t42 $env k15) (void)))
                      ...
                      (set-then!
                       ...
                       (app*
                        (make-closure
                         ...
                         (make-env $env_t39))
                        k25)))
                    (make-env $env_t51 (i14 i14)))
                   k24))
                (make-env $env_t39))
               (env-ref $env_t52 $env k14)))
            (make-env $env_t52 (k14 (env-ref $env_t52 $env k14))))))
        (make-env $env_t52 (k14 k14)))))
    (make-env $env_t39))
   (make-closure
    (lambda ($env rv42) ((cps py-print) g$myList $halt))
    (make-env $env_t39)))))
