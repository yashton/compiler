(module pytoc-common
  ...
  ; basic expressions contain no sub-expressions
  (define (basic? exp)
    (or (basic-nonvar? exp) (symbol? exp)))
  ...
  (define struct (apply set '(set dict py-list* tuple)))
  
  (define (struct? exp)
  	(set-member? struct exp))
  ...
  (define (free exp)
    (match exp
      ...
      [`(,(or 'λ 'lambda) ,vars ,body)
      	;=>
      	(set-subtract (free body) (apply set vars))]
